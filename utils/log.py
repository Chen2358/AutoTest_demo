...
class Logger(object):
	...
	def __init__(self, logger_name='framework'):
		...
		#日志输出级别
		self.console_output_level = c.get('console_level') if c and c.get('console_handler') else 'WARNING'
		self.file_oupt_level = c.get('file_level') if c and c.get('file_level') else 'DEBUG'
	...
	def get_logger(self):
		...
		if not self.logger.handlers:		#避免重复日志
			...
			file_handler.setLevel(self.file_oupt_level)
